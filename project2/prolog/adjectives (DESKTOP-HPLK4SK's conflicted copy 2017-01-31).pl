a0(_,  lam(P,lam(X,and(eq(X,X),app(P,X))))) --> [blind];[bold].
a0(an, lam(P,lam(X,and(not(some(Y,s_supports(X,Y)),app(P,X)))))) --> [empty].
% adjectives
%
a0(a,  lam(P,lam(X,and(a_beige_1(X),app(P,X))))) --> [beige].
a0(a,  lam(P,lam(X,and(a_black_1(X),app(P,X))))) --> [black].
a0(a,  lam(P,lam(X,and(a_black_2(X),app(P,X))))) --> [black].
a0(a,  lam(P,lam(X,and(a_black_and_white_1(X),app(P,X))))) --> [black,-,and,-,white].
a0(a,  lam(P,lam(X,and(a_blonde_1(X),app(P,X))))) --> [blond]; [blonde]; [light-haired].
a0(a,  lam(P,lam(X,and(a_blue_1(X),app(P,X))))) --> [blue]; [bluish]; [blueish].
a0(a,  lam(P,lam(X,and(a_blurry_1(X),app(P,X))))) --> [bleary]; [blurred]; [blurry]; [foggy]; [fuzzy]; [hazy]; [muzzy].
a0(a,  lam(P,lam(X,and(a_brown_1(X),app(P,X))))) --> [brown]; [brownish]; [chocolate,-,brown]; [dark,-,brown].
a0(a,  lam(P,lam(X,and(a_brunet_1(X),app(P,X))))) --> [brunet]; [brunette].
a0(a,  lam(P,lam(X,and(a_colorful_1(X),app(P,X))))) --> [colorful]; [colourful].
a0(a,  lam(P,lam(X,and(a_dark_2(X),app(P,X))))) --> [dark].
a0(a,  lam(P,lam(X,and(a_dirty_1(X),app(P,X))))) --> [dirty]; [soiled].
a0(an, lam(P,lam(X,and(a_dirty_1(X),app(P,X))))) --> [unclean].
a0(a,  lam(P,lam(X,and(a_golden_1(X),app(P,X))))) --> [gilded]; [gilt]; [gold]; [golden].
a0(an, lam(P,lam(X,and(a_golden_1(X),app(P,X))))) --> [aureate].
a0(a,  lam(P,lam(X,and(a_gray_1(X),app(P,X))))) --> [grey]; [gray]; [greyish]; [grayish].
a0(a,  lam(P,lam(X,and(a_green_1(X),app(P,X))))) --> [green]; [greenish]; [light,-,green]; [dark,-,green].
a0(a,  lam(P,lam(X,and(a_grey_1(X),app(P,X))))) --> [grey]; [gray]; [greyish]; [grayish].
a0(a,  lam(P,lam(X,and(a_grey_4(X),app(P,X))))) --> [grey]; [gray].
a0(an, lam(P,lam(X,and(a_indigo_1(X),app(P,X))))) --> [indigo].
a0(a,  lam(P,lam(X,and(a_long_2(X),app(P,X))))) --> [long].
a0(a,  lam(P,lam(X,and(a_maroon_1(X),app(P,X))))) --> [maroon]; [brownish,-,red].
a0(a,  lam(P,lam(X,and(a_mauve_1(X),app(P,X))))) --> [mauve].
a0(a,  lam(P,lam(X,and(a_metal_1(X),app(P,X))))) --> [metallic]; [metal].
a0(an, lam(P,lam(X,and(a_ocher_1(X),app(P,X))))) --> [ocher]; [ochre].
a0(an, lam(P,lam(X,and(a_old_1(X),app(P,X))))) --> [old].
a0(an, lam(P,lam(X,and(a_orange_1(X),app(P,X))))) --> [orange]; [orangish].
a0(a, lam(P,lam(X,and(a_orange_1(X),app(P,X))))) --> [ginger].
a0(an, lam(P,lam(X,and(a_outdoor_1(X),app(P,X))))) --> [outdoor]; [out-of-door]; [outside].
a0(a,  lam(P,lam(X,and(a_pink_1(X),app(P,X))))) --> [pink]; [pinkish].
a0(a,  lam(P,lam(X,and(a_purple_1(X),app(P,X))))) --> [purple]; [violet]; [purplish].
a0(a,  lam(P,lam(X,and('a_red-grey_1'(X),app(P,X))))) --> [red,-,grey]; [red,-,gray]; [reddish,-,grey]; [reddish,-,gray].
a0(a,  lam(P,lam(X,and(a_red_1(X),app(P,X))))) --> [red]; [reddish]; [ruddy]; [blood-red]; [carmine]; [cerise]; [cherry]; [cherry-red]; [crimson]; [ruby]; [ruby-red]; [scarlet].
a0(a,  lam(P,lam(X,and(a_sad_1(X),app(P,X))))) --> [sad].
a0(a,  lam(P,lam(X,and(a_shallow_1(X),app(P,X))))) --> [shallow].
a0(a,  lam(P,lam(X,and(a_shaven_1(X),app(P,X))))) --> [shaven]; [shaved].
a0(a,  lam(P,lam(X,and(a_short_2(X),app(P,X))))) --> [short].
a0(a,  lam(P,lam(X,and(a_silver_3(X),app(P,X))))) --> [silver]; [silvery]; [silverish].
a0(an, lam(P,lam(X,and(a_silver_3(X),app(P,X))))) --> [argent].
a0(a,  lam(P,lam(X,and(a_spotted_1(X),app(P,X))))) --> [patched]; [spotty]; [spotted].
a0(a,  lam(P,lam(X,and(a_stone_1(X),app(P,X))))) --> [stone].
a0(a,  lam(P,lam(X,and(a_striped_1(X),app(P,X))))) --> [striped]; [stripy].
a0(a,  lam(P,lam(X,and(a_transparent_1(X),app(P,X))))) --> [crystalline]; [crystal, clear]; [limpid]; [lucid]; [pellucid]; [transparent].
a0(a,  lam(P,lam(X,and(a_wet_1(X),app(P,X))))) --> [wet].
a0(a,  lam(P,lam(X,and(a_white_1(X),app(P,X))))) --> [white].
a0(a,  lam(P,lam(X,and(a_white_2(X),app(P,X))))) --> [white].
a0(a,  lam(P,lam(X,and(a_wooden_1(X),app(P,X))))) --> [wooden].
a0(a,  lam(P,lam(X,and(a_yellow_1(X),app(P,X))))) --> [yellow]; [yellowish]; [xanthous].

% a0(_,  lam(P,lam(X,and(eq(X,X),app(P,X))))) --> [_].

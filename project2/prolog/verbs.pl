...
iv(prp,_,  lam(X,eq(X,X))) --> [drinking];[coming];[looking];[shining];[running];[playing];[staring];[covering];[barking];[walking];[chilling];[biting];[sleeping];[swimming];[biking];[writing];[smiling];[flying];[leaning];[standing];[sitting];[lying];[resting];[parking];[falling];[feeding].
iv(dcl,sg, lam(X,eq(X,X))) --> [drinks];[covers];[barks];[runs];[walks]:[smiles];[flies];[leans];[stands];[sits];[lies];[falls];[plays];[lays].
iv(dcl,pl, lam(X,eq(X,X))) --> [drink];[cover];[bark];[run];[walk]:[smile];[fly];[lean];[stand];[sit];[lie];[fall];[play];[lay].
iv(bse,_,  lam(X,eq(X,X))) --> [drank];[covered];[barked];[ran];[walked]:[smiled];[flew];[leaned];[stood];[sat];[lied];[fell];[played];[layed].
iv(psp,_,  lam(X,eq(X,X))) --> [drunk];[made];[parked];[covered];[barked];[run];[walked];[smiled];[flown];[leaned];[stood];[sat];[lied];[fallen];[played];[layed].
...
tv(prp,_,  lam(P,lam(X,app(P,lam(Y,not(eq(X,Y))))))) --> [using];[playing];[containing];[opening];[tying];[jumping];[facing];[opening];[crunching];[munching];[riding];[holding];[wearing];[chasing];[kissing];[touching];[tying];[eating];[drinking];[making];[spreading];[stuffing];[biting];[caressing];[cycling];[feeding];[drawing];[ploughing];[attacking];[placing];[sniffing];[carrying];[photographing];[stroking];[peeling].
tv(dcl,sg, lam(P,lam(X,app(P,lam(Y,not(eq(X,Y))))))) --> [uses];[contains];[rides];[wears];[chases];[kisses];[touches];[ties];[eats];[drinks];[makes];[stuffs];[carresses];[cycles];[draws];[ploughs];[attacks];[places];[sniffs];[carries];[photographs];[strokes];[peels].
tv(dcl,pl, lam(P,lam(X,app(P,lam(Y,not(eq(X,Y))))))) --> [use];[contain];[ride];[wear];[chase];[kiss];[touch];[tie];[eat];[drink];[make];[stuff];[carress];[cycle];[draw];[plough];[attack];[place];[sniff];[carry];[photograph];[stroke];[peel].
tv(bse,_,  lam(P,lam(X,app(P,lam(Y,not(eq(X,Y))))))) --> [used];[contained];[rode];[wore];[chased];[kissed];[touched];[tied];[ate];[drank];[made];[stuffed];[drew];[ploughed];[attacked];[placed];[sniffed];[carried];[photographed];[stroked];[peeled].
tv(psp,_,  lam(P,lam(X,app(P,lam(Y,not(eq(X,Y))))))) --> [used];[contained];[ridden];[worn];[chased]; [kissed];[touched];[tied];[eaten];[drunk];[made];[stuffed];[drawn];[ploughed];[attacked];[placed];[sniffed];[carried];[photographed];[stroked];[peeled].
